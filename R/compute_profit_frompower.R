...
compute_profit_frompower <- function(energy, year, price, price_inc, discount = 0.12) {
  ...
  yearprofit$net <- yearprofit$energy * (price * (price_inc ** (year - year[1])))
...
#' #' @param discount rate (default 0.12)
#' #' @return data frame with estimate of profit
#' compute_profit_frompower <- function(energy, year, price_vec, discount = 0.12) {
#'   # make sure values are reasonable
#'   if (length(energy) < 1) {
#'     return(NA)
#'   }
